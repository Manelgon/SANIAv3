...
export function PatientDetailView({ patient, onBack }: PatientDetailViewProps) {
    ...
    const handlePatientUpdated = async () => {
        ...
        const { data } = await supabase
            ...
            .maybeSingle();
        if (data) {
            setLocalPatient({ ...(data as any), portfolio_name: (data as any).portfolio?.name });
        }
        const { data: allergiesData } = await supabase
            .from('patient_allergies')
                    ...
                    <div className="flex items-end gap-6 border-b border-slate-200 px-6 overflow-x-auto scrollbar-hide">
                        {tabs.map((tab) => (
                            <button
                                ...
                                className={`pb-3 pt-4 text-xs font-bold tracking-widest uppercase transition-all relative whitespace-nowrap border-b-4 ${activeTab === tab.id
